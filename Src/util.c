...
uint16_t timeoutCntSerial =
	0;                // Timeout counter for Rx Serial command
uint8_t timeoutFlagSerial =
	0;                // Timeout Flag for Rx Serial command: 0 = OK, 1 = Problem detected (line disconnected or wrong Rx data)
...
void input_init(void) {
  ...
  #endif
  #ifdef AUX45_USE_USART
  usart_Tx_DMA_config(USART_AUX, (uint8_t*)&auxSerial, sizeof(auxSerial));
  #endif
...
void usart_rx_check(void) {
  ...
  pos = rx_buffer_len - dma_transfer_number_get(DMA_CH4);
  // Check change in received data
  if (pos != old_pos) {
	// Initialize the pointer with FeedbackRaw address
	ptr = (uint8_t *)&FeedbackRaw;
	// "Linear" buffer mode: check if current position is over previous one AND data length equals expected length
	if (pos > old_pos && (pos - old_pos) == Lights_len) {
	  // Copy data. This is possible only if FeedbackRaw is contiguous! (meaning all the structure members have the same size)
	  memcpy(ptr, &rx_buffer[old_pos], Lights_len);
	  // Process data
	  usart_process_data(&FeedbackRaw, &Feedback);
	} // "Overflow" buffer mode: check if data length equals expected length
	else if ((rx_buffer_len - old_pos + pos) == Lights_len) {
	  // First copy data from the end of buffer
	  memcpy(ptr, &rx_buffer[old_pos], rx_buffer_len - old_pos);
	  // Check and continue with beginning of buffer
	  if (pos > 0) {
		// Move to correct position in FeedbackRaw
		ptr += rx_buffer_len - old_pos;
		// Copy remaining data
		memcpy(ptr, &rx_buffer[0], pos);
	  }
	  // Process data
	  usart_process_data(&FeedbackRaw, &Feedback);
	}
 ...
 */
#ifdef SERIAL_FEEDBACK
void usart_process_data(SerialFeedback *Feedback_in,
						SerialFeedback *Feedback_out) {
  uint16_t checksum;
  if (Feedback_in->start == SERIAL_START_FRAME) {
	checksum =
		(uint16_t)(Feedback_in->start ^ Feedback_in->cmd1 ^ Feedback_in->cmd2
			^ Feedback_in->speedR_meas ^ Feedback_in->speedL_meas
			^ Feedback_in->batVoltage ^ Feedback_in->boardTemp
			^ Feedback_in->cmdLed);
 ...
 */
int8_t i2c_writeBytes(uint8_t slaveAddr,
					  uint8_t regAddr,
					  uint8_t length,
					  uint8_t *data) {
  ...
  i2c_slaveAddress = slaveAddr
	  << 1;        // Address is shifted one position to the left. LSB is reserved for the Read/Write bit.
 ...
 */
int8_t i2c_writeBit(uint8_t slaveAddr,
					uint8_t regAddr,
					uint8_t bitNum,
					uint8_t data) {
  uint8_t b;
 ...
 */
int8_t i2c_readBytes(uint8_t slaveAddr,
					 uint8_t regAddr,
					 uint8_t length,
					 uint8_t *data) {
  ...
  if (2 == i2c_nDABytes) {
	i2c_ackpos_config(MPU_I2C,
					  I2C_ACKPOS_NEXT);            // send ACK for the next byte
  }
 ...
 */
int8_t i2c_readBit(uint8_t slaveAddr,
				   uint8_t regAddr,
				   uint8_t bitNum,
				   uint8_t *data) {
  uint8_t b;
